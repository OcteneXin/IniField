...
public class ModFeatureUtil {
    public static void putLeafBlock(IWorldGenerationReader world, Random random, BlockPos pos, BlockStateProvider state, Set<BlockPos> leavesPos) {
        if (/*leavesPos.contains(pos) ||*/ !TreeFeature.isAirOrLeaves(world, pos))
            return;
    ...
    public static void makeLeafCircle(IWorldGenerationReader world, Random random, BlockPos centerPos, float radius, BlockStateProvider state, Set<BlockPos> leaves) {
        ...
        // the pos to be immutable causing mrultiple same BlockPos instances to exist.
...
//        }
     ...
     * This takes all variables for setting Branch
     */
//    public static void drawBresenhamBranch(ModAbstractTreeGen<? extends ModTreeConfig> generator, IWorld world, Random random, BlockPos from, BlockPos to, Set<BlockPos> state, MutableBoundingBox mbb, ModTreeConfig config) {
//        for (BlockPos pixel : getBresenhamArrays(from, to)) {
...
//				}
            ...
            for (int cz = 0; cz < depth; cz++) {
                ...
                if (world.isAreaLoaded(pos_,1)) {
                    ...
                    for (int cy = 0; cy < height; cy++) {
                        // blank space above?
                        if (!world.getBlockState(pos_.above(cy)).isAir()) {
                            flag = false;
                        }
                    }
                } else {
    ...
    public static boolean surroundedByAir(IWorldReader world, BlockPos pos) {
        for (Direction e : Direction.values()) {
            if (!world.getBlockState(pos.relative(e)).isAir()) {
                return false;
            }
        }
    ...
    public static boolean isNearSolid(IWorldReader world, BlockPos pos) {
        for (Direction e : Direction.values()) {
            if (world.getBlockState(pos.relative(e)).getMaterial().isSolid())
                return true;
        }
